 ...
 */
package com.framework.exocet.common.persistence;
...
public class Page<T> {
	...
	// bootstrap分页
	private String pageType;// 分页类型 B:bootstrap－table 默认为原分页方式
	...
	public Page(HttpServletRequest request, HttpServletResponse response,
			int defaultPageSize) {
		String no = "";// 设置页码参数（传递repage参数，来记住页码）
		String size ="";// 设置页面大小参数（传递repage参数，来记住页码大小）
		String orderBy ="";// 设置排序参数
		 
		no = StringUtils.isNotEmpty(request.getParameter("offset"))?request.getParameter("offset"):request.getParameter("pageNo");
		size = StringUtils.isNotEmpty(request.getParameter("limit"))?request.getParameter("limit"):request.getParameter("pageSize");
				
		if (StringUtils.isNotEmpty(no)){// 记录开始数				
		...
		if (StringUtils.isNumeric(size)) {
			CookieUtils.setCookie(response, "pageSize", size);// 每页记录数
		...
		// 设置排序参数				 
		orderBy = request.getParameter("sortOrder");
